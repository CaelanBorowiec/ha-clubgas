        ...
        for user in users:
            ...
            entities.append(
                ClubGasTripCostSensor(
                    ...
                    trip_fuel,
                    user_id,
                    user.get(CONF_USER_NAME, user_id),
                )
            )
...
class ClubGasTripCostSensor(CoordinatorEntity[ClubGasCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        self._attr_unique_id = (
            f"{entry.entry_id}_{brand}_{store_id}_trip_{user_id}"
        )
        self._attr_name = f"{station_name} trip cost"
        self._attr_device_info = DeviceInfo(
            identifiers={(DOMAIN, f"{entry.entry_id}_user_{user_id}")},
            name=user_name,
            manufacturer="Club Gas",
            model="Household driver",
        )
    ...
    def extra_state_attributes(self) -> dict[str, str | float | None]:
        ...
        station = self._get_station()
        user = self._get_user()
        if trip is None or station is None or user is None:
            return {}
        return {
            ATTR_BRAND: self._brand,
            ATTR_STORE_ID: self._store_id,
            ...
            ATTR_USER_NAME: user.get(CONF_USER_NAME, self._user_name),
            ATTR_MPG: user.get(CONF_MPG),
            ATTR_DISTANCE_MILES: station.distance_miles,
            ATTR_PRICE: station.prices.get(self._fuel_type),
    ...
    def _get_user(self) -> dict | None:
        for user in get_configured_users(self._entry):
            if user.get(CONF_USER_ID) == self._user_id:
                return user
        return None
